...
def resize_n_crop_img_coeff_and_param(img, lm, t, s, target_size=224., mask=None):
    w0, h0 = img.size
    w = (w0*s).astype(np.int32)
    h = (h0*s).astype(np.int32)
    left = (w/2 - target_size/2 + float((t[0] - w0/2)*s)).astype(np.int32)
    ...
    img = img.resize((w, h), resample=Image.BICUBIC)
    img = img.crop((left, up, right, below))
    param = np.array([left, up, right, below, w, h])
    if mask is not None:
        mask = mask.resize((w, h), resample=Image.BICUBIC)
        mask = mask.crop((left, up, right, below))
    ...
    return img, lm, mask, param

def resize_n_crop_img_param(img, lm, t, s, target_size=224., mask=None):
    w0, h0 = img.size
    w = (w0*s).astype(np.int32)
    h = (h0*s).astype(np.int32)
    left = (w/2 - target_size/2 + float((t[0] - w0/2)*s)).astype(np.int32)
...
def align_img_coeff_and_param(img, lm, lm3D, mask=None, target_size=224., rescale_factor=102.):
    """
    Return:
        transparams        --numpy.array  (raw_W, raw_H, scale, tx, ty)
        img_new            --PIL.Image  (target_size, target_size, 3)
    ...
    # processing the image
    img_new, lm_new, mask_new,params = resize_n_crop_img_coeff_and_param(img, lm, t, s, target_size=target_size, mask=mask)
    trans_params = np.array([w0, h0, s, t[0], t[1]], dtype = "object")

    return trans_params, img_new, lm_new, mask_new,params

def align_img_param(img, lm, lm3D, mask=None, target_size=224., rescale_factor=102.):
    w0, h0 = img.size
    ...
    # processing the image
    param = resize_n_crop_img_param(img, lm, t, s, target_size=target_size, mask=mask)
